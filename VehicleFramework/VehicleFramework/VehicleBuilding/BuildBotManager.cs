...
namespace VehicleFramework
{
    public static class BuildBotManager
    {
        ...
        public static IEnumerator SetupVFXConstructing(GameObject mv)
        {
            ...
            VFXConstructing vfxc = mv.EnsureComponent<VFXConstructing>();
            vfxc.timeToConstruct = 30f;
            ...
            vfxc.rBody = mv.GetComponent<Rigidbody>();
        ...
        public static IEnumerator BuildBotPathsHelper(GameObject mv)
        {
            SetupBuildBotBeamPoints(mv);
            yield return UWE.CoroutineHost.StartCoroutine(SetupVFXConstructing(mv));
            Bounds vbounds;
            if(mv.GetComponent<ModVehicle>() != null)
            {
                vbounds = mv.GetComponent<ModVehicle>().BoundingBox.EnsureComponent<MeshRenderer>().bounds;
            }
            else
            {
                vbounds = mv.GetComponentInChildren<MeshRenderer>().bounds;
            }
        ...
        public static IEnumerator SetupBuildBotPaths()
        {
            foreach (ModVehicle mv in VehicleBuilder.prefabs)
            {
                ...
                yield return BuildBotPathsHelper(mv.gameObject);
            }
        }
    }
}
