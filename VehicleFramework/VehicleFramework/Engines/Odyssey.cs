...
{
    public class OdysseyEngine : ModVehicleEngine
    {
        protected override float FORWARD_TOP_SPEED => 1250;
        protected override float REVERSE_TOP_SPEED => 750;
        protected override float STRAFE_MAX_SPEED => 600;
        protected override float VERT_MAX_SPEED => 750;

        protected override float FORWARD_ACCEL => FORWARD_TOP_SPEED / 2.5f;
        ...
        */
        ...
        protected override void DrainPower(Vector3 moveDirection)
        {
            ...
            MV.gameObject.EnsureComponent<VehicleRootComponents.PowerManager>().TrySpendEnergy(scalarFactor * basePowerConsumptionPerSecond * upgradeModifier * Time.fixedDeltaTime);
        }
    }
}
