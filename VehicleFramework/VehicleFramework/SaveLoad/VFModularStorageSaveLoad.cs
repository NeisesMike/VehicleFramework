...
namespace VehicleFramework.SaveLoad
{
    internal static class VFModularStorageSaveLoad
    {
        ...
        internal static IEnumerator DeserializeAllModularStorage(ModVehicle mv)
        {
            ...
            foreach (string slotID in mv.slotIDs)
            {
                if (mv.modules.equipment.TryGetValue(slotID, out InventoryItem result))
                {
                    ...
                    if (container != null && container.container != null)
                    {
                        UWE.CoroutineHost.StartCoroutine(LoadThisModularStorage(mv, container.container, slotID));
                    }
                }
            }
            yield break;
        ...
        {
            var thisStorage = SaveLoad.JsonInterface.Read<List<Tuple<TechType, float, TechType>>>(mv, GetSaveFileName(slotID));
            ...
            foreach (var item in thisStorage)
            {
                ...
                {
                    Logger.Error($"Failed to add storage item {thisItem.name} to modular storage in slot {slotID} for {mv.name} : {mv.subName.hullName.text}");
                ...
                if (item.Item2 >= 0)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Logger.Error($"Failed to load reload battery power for modular storage item {thisItem.name} to modular storage in slot {slotID} for {mv.name} : {mv.subName.hullName.text}");
                        Logger.Log(e.Message);
                        Logger.Log(e.StackTrace);
                    }
                }
