...
namespace CricketVehicle
{
    ...
    public static class CellManagerPatcher
    {
        ...
        {
            if (ent.GetComponent<CricketContainer>() != null && __instance.streamer?.globalRoot is null)
            {
                // Sometimes this function is called when streamer.globalRoot is null
                // So if we're a CricketContainer, we simple will not use this function.
                // It sets the parent of the CricketContainer, but in-game we don't want it to have a parent.
